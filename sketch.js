const Engine = Matter.Engine;
const Render = Matter.Render;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
const Body = Matter.Body;
const Composites = Matter.Composites;
const Composite = Matter.Composite;

let engine;
let world;
var ground, bridge;
var leftWall, rightWall;
var jointPoint;
var jointLink;
var zombieImg, backgroundImg, zombie, breakButton;
var breakButtonImg;

var stones = [];

function preload() {

  zombieImg = loadImage('zombie_image.png');
  backgroundImg = loadImage('background_image.png');
  breakButtonImg = loadImage('axe_image.png')
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  engine = Engine.create();
  world = engine.world;
  frameRate(80);

  ground = new Base(0, height - 10, width * 2, 20, "#795548", true);
  leftWall = new Base(100, height - 300 , 200, height / 2 + 100, "#8d6e63", true);
  rightWall = new Base(width - 100, height - 300, 200, height / 2 + 100, "#8d6e63", true);

  bridge = new Bridge(30, { x: 50, y: height / 2 - 140 });
  jointPoint = new Base(width - 250, height / 2 - 100, 40, 20, "#8d6e63", true);

  Matter.Composite.add(bridge.body, jointPoint);
  jointLink = new Link(bridge, jointPoint);

  for (var i = 0; i <= 8; i++) {
    var x = random(width / 2 - 200, width / 2 + 300);
    var y = random(-100, 100);
    var stone = new Stone(x, y, 80, 80);
    stones.push(stone);
  }


  zombie = createSprite(width / 2, height - 110);
  zombie.addImage("zombie", zombieImg);
  zombie.scale = 0.1;
  zombie.velocityX = 10;

  breakButton = createButton("breakButtonImg");
  breakButton.position(width - 200, height / 2 - 50);
  breakButton.class("breakbutton");
  breakButton.mousePressed(handleButtonPress);
}
  // add bridgebody and join body by using hint 2
 
  

  //add for loop for creating the stones 
  
  

function draw() {
  background(backgroundImg);
  //image(backgroundImg, width/2, height/2, 500, 700);

  Engine.update(engine);

  ground.show();
  //leftWall.show();
  //rightWall.show();
  bridge.show();
  

  
  for (var stone of stones) {
    stone.show();
  }
  drawSprites();
}

function handleButtonPress() {
 jointLink.detach();
 setTimeout(() => {
   bridge.break();
 }, 1500);
}
